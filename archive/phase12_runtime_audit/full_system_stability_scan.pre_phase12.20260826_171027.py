...
if not check(
    "Phase 9 qualified list",
    isinstance(p9_list, list) and len(p9_list) == 29,
...
checks10 = [
    ("Phase 10 input", p10_input == 29, p10_input),
    ("Phase 10 evaluated", p10_eval == 29, p10_eval),
    ("Phase 10 monitoring", p10_monitoring == 9, p10_monitoring),
    ("Phase 10 closed", p10_closed == 20, p10_closed),
    ("Phase 10 price errors", p10_errors == 0, p10_errors),
    (
        "Phase 10 result count",
        isinstance(p10_results, list) and len(p10_results) == 29,
        len(p10_results) if isinstance(p10_results, list) else "INVALID"
    ),
]

for label, condition, detail in checks10:
    if not check(label, condition, str(detail)):
        fails += 1
...
if not check(
    "Phase 11 status",
    p11.get("status") == "VERIFICATION_PASSED",
    str(p11.get("status"))
):
    fails += 1
...
if not check(
    ...
    wake_ok,
    "AVAILABLE" if wake_ok else "NOT FOUND"
):
    fails += 1
